...
import { taskadapter } from "./internal/initialize";
...
export class TeamsBot extends TeamsActivityHandler {
  ...
  handleTeamsTaskModuleFetch(context, taskModuleRequest) {
    ...
    if (cardTaskFetchValue === TaskModuleIds.AdaptiveCard) {
        ...
        body: [
            {
                ...
                text: 'Task Module SSO'
            },
            {
                type: 'TextBlock',
                id: 'usertext',
                text: `You're logged in as ${userdata.getme().displayName} (${userdata.getme().userPrincipalName})${
                  userdata.getme().jobTitle ? `; your job title is: ${userdata.getme().jobTitle}` : ""
                    }.`,
            }
        ]
    });
  ...
  //multi task
  async multitask(context: TurnContext) {
    console.log("send multi SSO tasks...");
    const target = new TeamsBotInstallation (
      taskadapter,
      context.activity
    );
